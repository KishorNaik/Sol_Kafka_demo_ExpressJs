...
export class PubSubConsumerKafka {
    ...
    async subscribeMessageAsync<T>(onMessage: (msg: PubSubMessageKafka<T>) => Promise<void>): Promise<void> {
        ...
        await this.consumer.run({
            eachMessage: async ({ message }) => {
                ...
                const parsedMessage = JSON.parse(message.value.toString()).data as PubSubMessageKafka<T>;
